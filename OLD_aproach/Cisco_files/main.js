...
function onResizeListener() {
  ...
  if (isMobile) ratio = 2;
  if (ratio > 1) sc = 1.5;
  if (stage) stage.scaleX = stage.scaleY = ratio; 
  if (canvas) {
...
$('.modalbox .sections .options .button').click(function(){
						if ($(this).parents('.options').hasClass('open')){
							...
							$(this).parents('.options').find('.description-large').animate({'height':'hide'}, function(){ $('#irm-infographic').removeClass('open_infographic'); });
							}
						} else {
							if (isMobile) {
